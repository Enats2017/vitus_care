  ...
  try {
    ...
    if (lat && lng) {
      ...
      const placesResponse = await axios.get(
        ...
        { params: { input, key: process.env.MAP_API_KEY ,components: "country:IN",} }
      );
...
const applyJob = async (req, res) => {
  try {
    ...
    const filePath = `/uplode/${req.file.filename}`;
  ...
  try {
    ...
    if (lat && lng) {
      ...
      const placeDetailsResponse = await axios.get(
        `${process.env.GOOGLE_MAPS_BASE_URL}/place/details/json`,
        {
          params: { place_id: placeId, key: process.env.MAP_API_KEY,components: "country:IN", },
        }
      );
